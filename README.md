...
**内核模块无法事后安装。** vermagic 的算法是 `grep '=[ym]' .config | sort | md5`（`include/kernel-defaults.mk` 第 130 行），自编译的内核配置必然与官方不同，所以官方 kmods 源里的包在依赖检查阶段就会被拒绝。凡是用得到的模块必须在编译时就选进去。当前配置额外包含了 `kmod-tun`、`kmod-inet-diag`、`kmod-netlink-diag`、`kmod-wireguard`、`kmod-sched-cake` 作为余量。
